...
import { BehaviorSubject, Subject, catchError, of, retry, take } from 'rxjs';
...
export class F1Service {
  ...
  getSeasons() {
    this.http
      .get<SeasonsResponse>('https://ergast.com/api/f1/seasons.json')
      .pipe(
        retry(2),
        take(1),
        catchError((error) => {
          console.error(error);
          return of({
            //do this sane default rather in a separate file
            MRData: {
      ...
      .subscribe((response) => {
        const seasons = response.MRData.SeasonTable.Seasons.map(
